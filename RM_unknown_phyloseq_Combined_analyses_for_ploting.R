...
MUD.otu=read.table("D:/MUD_SequenceData/fwd_reads_demux_nophix_cut_fil_OTU_table.txt", header=T, row.names = 1)
MUD.map=import_qiime_sample_data("D:/MUD_SequenceData/MUB_map_both.txt")
MUD.OTU = otu_table(MUD.otu, taxa_are_rows = TRUE)
MUD.fung.tax=as.matrix(read.table("D:/MUD_SequenceData/its_MUD_sintax_simple_80.txt",header=T))
MUD.fung.TAX = tax_table(MUD.fung.tax)
MUD.data=phyloseq(MUD.OTU,sample_data(MUD.map),MUD.fung.TAX)
rep_set.fung<- read.fasta(file = "D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/uniques_fwd_reads_demux_nophix_cut_fil_otus.fa", as.string = TRUE, set.attributes = FALSE)
...
MUD_poss_bact=read.delim("D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/MUD_Galaxy_Kraken-filter_bacteria.txt",
                         sep = "\t", header = F)
head(MUD_poss_bact)
...
#149
write.fasta(sequences =rep_set_Kracken_bacteria, names = names(rep_set_Kracken_bacteria), file.out ="D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/MUD_Kranken_bacteria_rep_set.fna")

#Read in hit table 

ncbi_kracken=read.csv("D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/Galaxy_generated_taxonomy/K3DG24NT01R-Alignment-HitTable.csv",
                      header = F)
head(ncbi_kracken)
nrow(ncbi_kracken)
ncbi_kracken[,c(1,2)]
...
ncbi_kracken_uq <- ncbi_kracken[!duplicated(ncbi_kracken[,"V1"]),]
nrow(ncbi_kracken_uq)
#149
genbank2uid(id = "NR_158052.1")

for (i in seq(nrow(ncbi_kracken_uq))) {
  ncbi_kracken_uq$uid[i]=genbank2uid(id = (as.character(ncbi_kracken_uq[i,2])))
  
  
}
classification((ncbi_kracken_uq$uid[[1]]))
attributes(ncbi_kracken_uq$uid[[1]])



ncbi_kracken_taxID=genbank2uid(id = c(as.character(ncbi_kracken_uq[,2])))
length(ncbi_kracken_taxID)
ncbi_kracken_taxID[[1]]
attributes(ncbi_kracken_taxID[[1]])
classification(ncbi_kracken_taxID[[1]])
length(ncbi_kracken_taxID)



list_new=c()
for (i in seq(length((ncbi_kracken_uq$uid)))) {
  list_new[i]=classification((ncbi_kracken_uq$uid[[i]]))
  for (j in seq(length(list_new))) {
    ncbi_kracken_uq$King[i] =list_new[[j]][2,1]
...
ncbi_kracken_uq$King

ncbi_kracken_uq_bact=subset(ncbi_kracken_uq,King=="Bacteria")
nrow(ncbi_kracken_uq_bact)
#137
MUD.Fung_tax_names=taxa_names(MUD.Fung)
length(MUD.Fung_tax_names)
MUD.Fung_tax_names_not_bact<-MUD.Fung_tax_names[MUD.Fung_tax_names %w/o% ncbi_kracken_uq_bact$V1]
length(MUD.Fung_tax_names_not_bact)
save(MUD.Fung_tax_names_not_bact, file = "D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/Galaxy_generated_taxonomy/List_not_bact_OTUs.Rdata")
#####KRACKEN Bact Search####
load(file = "D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/Galaxy_generated_taxonomy/List_not_bact_OTUs.Rdata")

MUD.Fung_no_bact=prune_taxa(MUD.Fung_tax_names_not_bact, MUD.Fung)


ntaxa(MUD.Fung_no_bact)
#4839
sum(otu_table(MUD.Fung_no_bact))
#4234602

sort(sample_sums(MUD.Fung_no_bact))
#Min=7815
#Max=79420

#now I want to look at the taxa classification that were id as bacteria using KRACKEN

MUD.Fung_bact=prune_taxa(as.character(ncbi_kracken_uq_bact$V1), MUD.Fung)
tax_table(MUD.Fung_bact)
#They look good, only one taxa was classified below Fungi
#OTU3761 "d:Fungi" "p:Basidiomycota" "UNKNOWN" "UNKNOWN" "UNKNOWN" "UNKNOWN" "UNKNOWN"

#Now I want to export unknown sequences and blast them to see if there are any crappy seq or lingering bacteria
MUD.Fung_no_bact_unk=subset_taxa(MUD.Fung_no_bact, Phylum=="UNKNOWN")
ntaxa(MUD.Fung_no_bact_unk)
#1820
sum(otu_table(MUD.Fung_no_bact_unk))
#884624


#I am going to export the rep set from this and blast them 
rep_set_unknown=rep_set.fung[names(rep_set.fung) %in% taxa_names(MUD.Fung_no_bact_unk)]
head(rep_set_unknown)
length(rep_set_unknown)
#1820
write.fasta(sequences =rep_set_unknown, names = names(rep_set_unknown), file.out ="D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/MUD_unknown_phyla_rep_set.fna")
...
# I had to split the FNA into two sets for blasting 

#First half
#Read in hit table 


ncbi_UNKNOWN_1st_half=read.csv("D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/UNKNOWN_taxa/K4BF369E014-Alignment-HitTable.csv",
                      header = F)
head(ncbi_UNKNOWN_1st_half)
nrow(ncbi_UNKNOWN_1st_half)
#4306
ncbi_UNKNOWN_1st_half[,c(1,2)]

#I only care about the top value
ncbi_UNKNOWN_1st_half_uq <- ncbi_UNKNOWN_1st_half[!duplicated(ncbi_UNKNOWN_1st_half[,"V1"]),]
nrow(ncbi_UNKNOWN_1st_half_uq)
#464


for (i in seq(nrow(ncbi_UNKNOWN_1st_half_uq))) {
  ncbi_UNKNOWN_1st_half_uq$uid[i]=genbank2uid(id = (as.character(ncbi_UNKNOWN_1st_half_uq[i,2])))
  
  
}
classification((ncbi_UNKNOWN_1st_half_uq$uid[[1]]))
attributes(ncbi_UNKNOWN_1st_half_uq$uid[[1]])



ncbi_UNKNOWN_1st_half_taxID=genbank2uid(id = c(as.character(ncbi_UNKNOWN_1st_half_uq[,2])))
length(ncbi_UNKNOWN_1st_half_taxID)
ncbi_UNKNOWN_1st_half_taxID[[1]]
attributes(ncbi_UNKNOWN_1st_half_taxID[[1]])
classification(ncbi_UNKNOWN_1st_half_taxID[[1]])
length(ncbi_UNKNOWN_1st_half_taxID)



list_new=c()
for (i in seq(length((ncbi_UNKNOWN_1st_half_uq$uid)))) {
  list_new[i]=classification((ncbi_UNKNOWN_1st_half_uq$uid[[i]]))
  for (j in seq(length(list_new))) {
    ncbi_UNKNOWN_1st_half_uq$King[i] =list_new[[j]][2,1]
...
ncbi_UNKNOWN_1st_half_uq$King
#summary(ncbi_UNKNOWN_1st_half_uq)

#subset to only Eukaryota
ncbi_UNKNOWN_1st_half_uq_Euk=subset(ncbi_UNKNOWN_1st_half_uq,King=="Eukaryota")
nrow(ncbi_UNKNOWN_1st_half_uq_Euk)
#310

#Now I want to remove any reads that blasted as non fungi
head(ncbi_UNKNOWN_1st_half_uq_Euk)

list_new=c()
for (i in seq(length((ncbi_UNKNOWN_1st_half_uq_Euk$uid)))) {
  list_new[i]=classification((ncbi_UNKNOWN_1st_half_uq_Euk$uid[[i]]))
  for (j in seq(length(list_new))) {
    ncbi_UNKNOWN_1st_half_uq_Euk$King_2[i] =list_new[[j]][4,1]
    
  }
  
}

unique(ncbi_UNKNOWN_1st_half_uq_Euk$King_2)
#subset to only fungi
ncbi_UNKNOWN_1st_half_uq_fungi=subset(ncbi_UNKNOWN_1st_half_uq_Euk,King_2=="Fungi"|King_2=="Opisthokonta incertae sedis")
nrow(ncbi_UNKNOWN_1st_half_uq_fungi)
#232
...
ncbi_UNKNOWN_2nd_half=read.csv("D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/UNKNOWN_taxa/K4B5352Y014-Alignment-HitTable.csv",
                               header = F)
head(ncbi_UNKNOWN_2nd_half)
nrow(ncbi_UNKNOWN_2nd_half)
#5292
ncbi_UNKNOWN_2nd_half[,c(1,2)]

#I only care about the top value
ncbi_UNKNOWN_2nd_half_uq <- ncbi_UNKNOWN_2nd_half[!duplicated(ncbi_UNKNOWN_2nd_half[,"V1"]),]
nrow(ncbi_UNKNOWN_2nd_half_uq)
#580


for (i in seq(nrow(ncbi_UNKNOWN_2nd_half_uq))) {
  ncbi_UNKNOWN_2nd_half_uq$uid[i]=genbank2uid(id = (as.character(ncbi_UNKNOWN_2nd_half_uq[i,2])))
  
  
}
classification((ncbi_UNKNOWN_2nd_half_uq$uid[[1]]))
attributes(ncbi_UNKNOWN_2nd_half_uq$uid[[1]])



ncbi_UNKNOWN_2nd_half_taxID=genbank2uid(id = c(as.character(ncbi_UNKNOWN_2nd_half_uq[,2])))
length(ncbi_UNKNOWN_2nd_half_taxID)
ncbi_UNKNOWN_2nd_half_taxID[[1]]
attributes(ncbi_UNKNOWN_2nd_half_taxID[[1]])
classification(ncbi_UNKNOWN_2nd_half_taxID[[1]])
length(ncbi_UNKNOWN_2nd_half_taxID)



list_new=c()
for (i in seq(length((ncbi_UNKNOWN_2nd_half_uq$uid)))) {
  list_new[i]=classification((ncbi_UNKNOWN_2nd_half_uq$uid[[i]]))
  for (j in seq(length(list_new))) {
    ncbi_UNKNOWN_2nd_half_uq$King[i] =list_new[[j]][2,1]
...
unique(ncbi_UNKNOWN_2nd_half_uq$King)


#subset to only Eukaryota
ncbi_UNKNOWN_2nd_half_uq_Euk=subset(ncbi_UNKNOWN_2nd_half_uq,King=="Eukaryota")
nrow(ncbi_UNKNOWN_2nd_half_uq_Euk)
#568

#Now I want to remove any reads that blasted as non fungi
head(ncbi_UNKNOWN_2nd_half_uq_Euk)

list_new=c()
for (i in seq(length((ncbi_UNKNOWN_2nd_half_uq_Euk$uid)))) {
  list_new[i]=classification((ncbi_UNKNOWN_2nd_half_uq_Euk$uid[[i]]))
  for (j in seq(length(list_new))) {
    ncbi_UNKNOWN_2nd_half_uq_Euk$King_2[i] =list_new[[j]][4,1]
    
  }
  
}

unique(ncbi_UNKNOWN_2nd_half_uq_Euk$King_2)
#subset to only fungi
ncbi_UNKNOWN_2nd_half_uq_fungi=subset(ncbi_UNKNOWN_2nd_half_uq_Euk,King_2=="Fungi"|King_2=="Opisthokonta incertae sedis")
nrow(ncbi_UNKNOWN_2nd_half_uq_fungi)
#499

#Now combine the two half to create the combined list of unknown fungi that are fungi
ncbi_UNKNOWN_uq_fungi_otu=c(as.character(ncbi_UNKNOWN_2nd_half_uq_fungi$V1),as.character(ncbi_UNKNOWN_1st_half_uq_fungi$V1))
length(ncbi_UNKNOWN_uq_fungi_otu)
#731

#Now create a list of OTUs that need to be removed from the dataset

MUD.Fung_no_bact_unk_OTU_names=taxa_names(MUD.Fung_no_bact_unk)
length(MUD.Fung_no_bact_unk_OTU_names)
#1820
MUD.Fung_tax_names_unk_not_fungi<-MUD.Fung_no_bact_unk_OTU_names[MUD.Fung_no_bact_unk_OTU_names %w/o% ncbi_UNKNOWN_uq_fungi_otu]
length(MUD.Fung_tax_names_unk_not_fungi)
#1089

MUD.Fung_no_bact_tax_names=taxa_names(MUD.Fung_no_bact)
length(MUD.Fung_no_bact_tax_names)
#4839

MUD.Fung_no_bact_tax_names_fungi<-MUD.Fung_no_bact_tax_names[MUD.Fung_no_bact_tax_names %w/o% MUD.Fung_tax_names_unk_not_fungi]
...
write.table(tax_table(MUD.Fung_fungi), "D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/MUD_fungi_fungi_taxon_ITS_trunc_phyl.txt") 
write.table(otu_table(MUD.Fung_fungi), "D:/MUD_SequenceData/Analyses_collaboration/MUD_Transition/MUD_fungi_fungi_OTU_ITS_trunc_phyl.txt") 
